...
PAGINA_INICIO = 26
PAGINA_FIM = 42  # 17 páginas × 28 termos × ~7 novas por página ≈ 3.300 novas
...
def coletar_termo(termo, pagina_inicio, pagina_fim):
    conn = conectar_banco()
    cursor = conn.cursor()
    ...
    for pagina in range(pagina_inicio, pagina_fim + 1):
        print(f"  Página {pagina}/{pagina_fim}...", end=" ")

        url = "https://www.tjrs.jus.br/buscas/jurisprudencia/ajax.php"

        parametros = (
            f"aba=jurisprudencia&realizando_pesquisa=1&pagina_atual={pagina}"
            ...
            f"&facet=on&facet.sort=index&facet.limit=index&wt=json&ordem=desc&start={(pagina-1)*10}"
        )
...
for i, termo in enumerate(TERMOS, 1):
    print(f"\n[{i}/{len(TERMOS)}] Coletando: '{termo}' (páginas {PAGINA_INICIO}-{PAGINA_FIM})")
    salvas, duplicadas = coletar_termo(termo, PAGINA_INICIO, PAGINA_FIM)
    total_geral += salvas
